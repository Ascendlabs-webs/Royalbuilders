...
export default function CrmPage() {
  ...
  const streetFileRef = useRef(null);
      ...
      if (validateStep(currentStep)) {
        ...
        const newProp = {
          ...
          photo: previewMainImg
            ? URL.createObjectURL(mainFileRef.current.files[0])
  ...
  const addSellingPoint = (e) => {
    e.preventDefault();
    const input = document.querySelector('.selling-point-input input');
  ...
  const handleStreetFileChange = (e) => {
  ...
  // Effect to clean up object URLs on unmount (optional, but good practice)
  useEffect(() => {
    return () => {
      if (previewMainImg) URL.revokeObjectURL(previewMainImg);
      // Additionally revoke any other object URLs if stored
    };
  }, [previewMainImg]);
